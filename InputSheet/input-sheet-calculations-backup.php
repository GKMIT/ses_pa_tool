...
move_uploaded_file($_FILES['input_sheet']['tmp_name'],$file_name);
...
for($i=3;$i<=22;$i++) {
    ...
    if($din_no!="" && !strpos($d_name,"*")) {
        $db->registerNewDirector($din_no,$dir_name);
        echo $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getValue()." , ".$dir_name."<br/>";
    }
    else
        break;
}
...
<table width="100%" border="1">
    ...
    for($i=3;$i<=22;$i++) {
        ...
        if($din_no!="" && !strpos($din_no,"*")) {
            ...
            $info['total_association']=$objPHPExcel->getActiveSheet()->getCell('F'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('F'.$i)->getValue();
            $info['classification'] = $objPHPExcel->getActiveSheet()->getCell('G'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('G'.$i)->getValue();
            ...
            $info['remuneration'] = $objPHPExcel->getActiveSheet()->getCell('K'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('K'.$i)->getValue();
            $info['nomination'] = $objPHPExcel->getActiveSheet()->getCell('L'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('L'.$i)->getValue();
            $info['shares_held'] = $objPHPExcel->getActiveSheet()->getCell('M'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('M'.$i)->getValue();
            ...
            $info['last_3_agm_attendance'] = $objPHPExcel->getActiveSheet()->getCell('P'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('P'.$i)->getValue();
            $info['no_directorship'] = $objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('Q'.$i)->getValue();
            $info['committee_memberships'] = $objPHPExcel->getActiveSheet()->getCell('R'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('R'.$i)->getValue();
            $info['committee_chairmanships'] = $objPHPExcel->getActiveSheet()->getCell('S'.$i)->getValue()=="" ? "" : $objPHPExcel->getActiveSheet()->getCell('S'.$i)->getValue();
...
<table width="100%" border="1">
    ...
    for($i=24;$i<=43;$i++) {
        ...
        if($d_name!="" && !strpos($d_name,"*")) {
            ...
            $year_counter = 0;
            for($k=C;$k<=M;$k++) {
                echo "<td>".round($objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue(),2)."</td>";
                $info['com_bse_code'] = $bse_code;
                ...
                $info['variable_pay']=round($objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue(),2);
                $k++;
                $info['fixed_pay']=round($objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue(),2);
                $db->registerDirectorRenumeration($info);
            }
            echo "</tr>";
        }
        else
    ...
    for($i=45;$i<=64;$i++) {
        ...
        if($d_name!="" && !strpos($d_name,"*")) {
            ...
            for($k=C;$k<=M;$k++) {
                ...
                $db->directorBoardAttendance($info);
            }
            echo "</tr>";
        }
        else
...
<p>remunerationCommitteeAttendance</p>
    ...
    for($i=87;$i<=106;$i++) {
        ...
        if($d_name!="" && !strpos($d_name,"*")) {
            ...
            for($k=C;$k<=M;$k++) {
                echo "<td>".$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue()."</td>";
                $info['com_bse_code'] = $bse_code;
                $dir_cell = intval($i)-84;
                $info['dir_din_no'] = $objPHPExcel->getActiveSheet()->getCell('A'.$dir_cell)->getValue();
                $info['att_year']=$ren_years[$year_counter++];
                $info['attended']=$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue() == "" ? 0 : $objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue();
                $k++;
                $info['held']=$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue() == "" ? 0 : $objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue();
                $db->remunerationCommitteeAttendance($info);
            }
            echo "</tr>";
        }
    }
...
<p>Nomination Committee Attendance</p>
    ...
    for($i=108;$i<=127;$i++) {
        ...
        if($d_name!="" && !strpos($d_name,"*")) {
            ...
            for($k=C;$k<=M;$k++) {
                echo "<td>".$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue()."</td>";
                $info['com_bse_code'] = $bse_code;
                $dir_cell = intval($i)-105;
                $info['dir_din_no'] = $objPHPExcel->getActiveSheet()->getCell('A'.$dir_cell)->getValue();
                $info['att_year']=$ren_years[$year_counter++];
                $info['attended']=$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue() == "" ? 0 : $objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue();
                $k++;
                $info['held']=$objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue() == "" ? 0 : $objPHPExcel->getActiveSheet()->getCell($k.$i)->getValue();
                $db->nominationCommitteeAttendance($info);
            }
            echo "</tr>";
        }
    }
